...
In Kubernetes v1.36, the `ExternalServiceAccountTokenSigner` feature for service accounts graduates to stable, 
...
so existing RBAC and authentication flows continue to work as expected. 
...
With GA in v1.36, `ExternalServiceAccountTokenSigner` is now a fully supported option for platforms that 
...
The transition of Admin Access to GA provides a permanent, secure framework for cluster administrators 
...
graduation of Mutating Admission Policies to Stable. This milestone provides a native, 
...
in Kubernetes v1.36 with the graduation of the Gogo Protobuf Removal to Stable. 
This initiative eliminates a significant dependency on the unmaintained `gogoprotobuf` library, 
...
The distribution of container data becomes more flexible in Kubernetes v1.36 as OCI Volume Sources graduate to Stable. 
...
In Kubernetes v1.36, the `ComponentStatusz` feature for core Kubernetes components graduates to beta, 
standardizing a `/statusz` endpoint that surfaces real‑time build and version details for each component. 
...
In Kubernetes v1.36, the `ComponentFlagz` feature for core Kubernetes components graduates to beta, 
...
The endpoint supports both a human‑readable text view and a versioned structured API (initially `config.k8s.io/v1alpha1`), 
so you can either curl it during an incident or wire it into automated tooling once you are ready. 
Access is restricted to the `system:monitoring` group and sensitive values can be redacted, 
...
Kubernetes now enhances MemoryQoS on cgroup v2 nodes with smarter, tiered memory protection that better aligns kernel 
...
combined with support for List Types in attributes, transform DRA from a low-level primitive into a robust system 
